<?php
	...
	session_start();
	if($_SESSION['this_ip']!=''){
		$this_ip_no_www=preg_replace('/^www\./i','',$_SESSION['this_ip']);
	} else {
		$this_ip_no_www=preg_replace('/^www\./i','',$_SERVER['HTTP_HOST']);	
	}

	define("CS_EMAIL_ROOT_PATH", str_replace(DIRECTORY_SEPARATOR.basename(__FILE__),'',__FILE__));
	...
	function SendSMTPEmail($toaddr, $fromaddr, $message, $options = array())
	{
		...
		if (!EmailAddressesToNamesAndEmail($tempfromnames, $tempfromaddrs, $fromaddr, true, $options)){
			//return array("success" => false, "error" => SMTP_Translate("Invalid 'From' e-mail address."), "info" => $fromaddr);
		}
	...
	function SohoEmail($toaddr, $fromaddr, $subject, $htmlmessage, $options = array()){
		global $this_ip_no_www;
	#### Example options
	//			$attachments_ar[] = array('name'=>getcwd()."/pop3.php");
	//			$smtpoptions = array(
	//				//"htmlmessage" => $body,
	//				//"attachments"=>$attachments_ar
	//				//"replytoaddr" => "noreply@validatedebt.com"
	//				//"textmessage" => ConvertHTMLToText($body),				
	//				//"secure" => false,
	//				//"returnresults"=>true,
	////				"usemail" => false
	//				
	//				//"server" => 'localhost',
	//				//"port" => 25,
	//				//"ccaddr"=>"example@example,somebodyelse@somewhere.com",
	//				//"bccaddr"=>"example@example,somebodyelse@somewhere.com",
	//				//"username" => "tunkklhk"
	////				"password" => "kq666Uiu0V*"
	//			);
		if(preg_match('/,/',$toaddr)){
			$toaddr_ar=explode(',',$toaddr);
			foreach($toaddr_ar as $var=>$val){
				if($var==0){
					$toaddr=$val;
				} else {
					$options['ccaddr'] .= ','.$val;
			...
			$options['ccaddr']=preg_replace('/^,/','',$options['ccaddr']);
		}
		
		$toaddr=trim(preg_replace('/[^\<\>\.+@_a-z0-9-]/i','',$toaddr));
		$whoami=get_current_user();
		if($whoami==''){
			if(shell_exec('echo hi') == '') { $whoami = exec("whoami"); } else { $whoami = shell_exec("whoami"); }
			if($whoami==''){ $whoami_ar = posix_getpwuid(fileowner(__FILE__)); $whoami= $whoami_ar['name']; }
		}
		$server_address=$this_ip_no_www."<".str_replace("\n", '', $whoami)."@".php_uname("n").">";
		if($fromaddr==''){ $fromaddr=$server_address; }
		if(!preg_match('/[@]/',$fromaddr)){ $fromaddr=$fromaddr."<".str_replace("\n", '', $whoami)."@".php_uname("n").">"; }
		$fromaddr=trim($fromaddr);
		$options["htmlmessage"]= $htmlmessage;
		...
		if($options["usemail"] != true){
			$headers = (isset($options["headers"]) ? $options["headers"] : GetEmailUserAgent("Thunderbird"));
		} else {
			$headers = "";	
		}
		
		$textmessage = (isset($options["textmessage"]) ? $options["textmessage"] : ConvertHTMLToText($options["htmlmessage"]));
		...
		if(count($attachments)>0){
			unset($attachments_ar);
			foreach($attachments as $pval){
				$attachments_ar[] = array('name'=>$pval);	
			} 			
			$attachments = $attachments_ar;
			foreach($attachments as $afv=>$afile){
				if($attachments[$afv]['name']!=''){
					if(!file_exists($attachments[$afv]['name'])){						
						unset($attachments[$afv]);
					}
				}
			}	
		}
//		echo testArray($attachments);
//		exit;
		...
		if($messagefromaddr==''){ $messagefromaddr='From: '.$fromaddr."\r\n"; }
		...
		else
		{
			...
			$destheaders = "";
			if ($headers != ""){  $destheaders .= $headers; }			
			$destheaderssmtp = $destheaders."MIME-Version: 1.0\r\n".$messagetoaddr;
			$destheadersdef = $destheaders."MIME-Version: 1.0\r\n";
			$destheaders = "";
//			if($options["usemail"]!=true){
//				$destheaders .= $messagetoaddr;
//			}
			...
			for ($x = 0; $x < $y; $x++){
				$mimecontent .= "--" . $mimeboundary . "\r\n";
				if($attachments[$x]["name"]!=''){
					if(preg_match('/\.(gif|png|jpeg|jpg|bmp|tif|tiff|ico)$/i',$attachments[$x]["name"])){
						//$disposition='inline';
						$disposition='attachment'; 
						$img_dim = getimagesize($attachments[$x]["name"]);
						$attachments[$x]["type"]=$img_dim['mime'];
					} else { 
						$disposition='attachment'; 
						$attachments[$x]["type"]='application/octet-stream';
					}
					if($attachments[$x]["type"]==''){ $attachments[$x]["type"]='application/octet-stream'; }
					if($attachments[$x]["data"]==''){ $attachments[$x]["data"]=file_get_contents($attachments[$x]["name"]); }
					$attachments[$x]["name"]=basename($attachments[$x]["name"]);
				}
				...
				if ($name != "")  $mimecontent .= "Content-Disposition: ".$disposition."; filename=\"" . $name . "\"\r\n";
				$mimecontent .= "\r\n";
		...
		if (isset($options["returnresults"]) && $options["returnresults"]){
			return array("toaddr" => $toaddr, "fromaddr" => $fromaddr, "headers" => $messagefromaddr.$destheaderssmtp.$destheaders, "subject" => $subject, "message" => $message);
		} elseif(isset($options["usemail"]) && $options["usemail"]){			
			if(!$mail_rez = mail($toaddr, $subject, $message, $messagefromaddr.$destheadersdef.$destheaders)){
//				if(shell_exec('echo hi') == '') { $whoami = exec("whoami"); } else { $whoami = shell_exec("whoami"); }
//				if($whoami==''){ $whoami_ar = posix_getpwuid(fileowner(__FILE__)); $whoami= $whoami_ar['name']; }
//				$server_address=str_replace("\n", '', $whoami)."@".php_uname("n");
				$messagefromaddr="From: ".$this_ip_no_www."<".$server_address.">\r\n";
				$mail_rez = mail($toaddr, $subject, $message, $messagefromaddr.$destheadersdef.$destheaders);
			}
			return $mail_rez;
		} else {
			$mail_rez = SendSMTPEmail($toaddr, $fromaddr, $messagefromaddr.$destheaderssmtp.$destheaders . "\r\n" . $message, $options);
			if($mail_rez['success']!=true){
				$options['replytoaddr']=$fromaddr;
				$mail_rez = SendSMTPEmail($toaddr, $server_address, "From: ".$server_address."\r\n".$destheaderssmtp.$destheaders . "\r\n" . $message, $options);
				if($mail_rez['success']!=true){
					if(!mail($toaddr, $subject, $message, $messagefromaddr.$destheadersdef.$destheaders)){
						if(mail($toaddr, $subject, $message, "From: ".$server_address."\r\n".$destheadersdef.$destheaders)){
							$mail_rez = array('success'=>true,'rawsend'=>'','rawrecv'=>'');
							//return true;
						} else {
							return false;
						}
