			...
			ed.addCommand('mcePDWToggleToolbars', function() {
				...
				for(j = 0; j < tbIds.length; j++){
					...
					obj = ed.controlManager.get(tbIds[j]);
	                    ...
	                    id = 'tiny_editor_toolbar1';
					var table=document.getElementById(id);
					var td= table.getElementsByTagName("td");
					for(var i=23;i<td.length-1;i++){						
					...
					var td= table.getElementsByTagName("td");
//					for(var i=23;i<td.length-1;i++){
					for(var i=25;i<td.length-1;i++){
						if (td[i].style.display=='none') {
							Toggle_PDW = 0;
							td[i].style.display='table-cell';
							ajaxDo(url + '/toggle_mode.php?tinymode=advanced', 0);
						} else {
							Toggle_PDW = 1;
							ajaxDo(url + '/toggle_mode.php?tinymode=basic', 0);	
							td[i].style.display='none';
						}
					}
				}
