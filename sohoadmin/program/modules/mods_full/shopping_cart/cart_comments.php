...
$result = mysql_query("SELECT BIZ_VERIFY_COMMENTS, DISPLAY_COMMENTS FROM cart_options LIMIT 1");
...
if ( $cartprefs->get("comments_required_approval") == "" ) { $cartprefs->set("comments_required_approval", "yes"); } // has to be something...650px was limit mentioned in note text b4 v4.9 r54
$cart_require_approval = $cartprefs->get("comments_required_approval");


if($enable_comments == 'Y'){
   $is_enabled = "checked";
}else{
   $is_enabled = "";
}

if($cart_require_approval == 'yes'){
...
function check_enable() {   
   ajaxDo('comment_result.php?process=comment_settings&is_enabled=Y', 'enabled_result');
}
...
function check_n_send() {
   ...
   if(document.getElementById('is_enabled').checked){ var is_enabled = "Y"; }else{ var is_enabled = "N"; }
	if(document.getElementById('require_approval').checked){ var require_approval = "yes"; }else{ var require_approval = "no"; }


   ajaxDo('comment_result.php?process=comment_settings&email_to='+email_to+'&is_enabled='+is_enabled+'&require_approval='+require_approval, 'settings_result');
}
...
function check_display(ele) {
   ...
   <div id="tab_interface_container" style="display: block; width: 100%;margin: 40px 5px 20px 5px;position: relative; border: 0px solid red;">
      ...
      <table id="tab1-content" border="0" cellspacing="0" cellpadding="0" class="feature_sub tab_content" style="display: table;">
         <tr>
		<td style="width:810px;padding-left: 15px;">
			
<?php
if($enable_comments != 'Y'){
	echo "<div id=\"enabled_result\" class=\"display_options\"><span style=\"color:red;font-weight:bold;\">* Warning:</span> Shopping Cart Comments are currently <span style=\"color:red;font-weight:bold;\">disabled</span>!  <a href=\"javascript:void(0);\" onclick=\"check_enable();\" style=\"bottom-border:1px;\">Click here</a> to enable cart comments.</div><br/>\n";
}
?>
			
			
               <div class="display_options">
                  <form name="display_form" action="cart_comments.php">
...
<?php
	...
	$display_what = "";
	if($display_new == "checked"){ $display_what .= "status = 'not_approved' "; }
...
//		
		...
		$comment_qry = "SELECT cart_comments.PRIKEY,cart_comments.PROD_ID, cart_comments.COMMENT_TITLE,cart_comments.COMMENT,cart_comments.RATING,cart_comments.NAME,cart_comments.LOCATION,cart_comments.COMMENT_DATE,cart_comments.STATUS,cart_comments.COMMENT_HTML,cart_comments.AUTH_KEY,cart_products.PROD_NAME FROM cart_comments left join cart_products on cart_products.PRIKEY=cart_comments.PROD_ID where ".$display_what." order by cart_comments.PRIKEY DESC";
		...
		if(mysql_num_rows($comment_result) > 0){
			...
			echo "<h2>".mysql_num_rows($comment_result)." new comments.</h2>\n";
			...
			while($COMMENTS = mysql_fetch_array($comment_result)){
				...
				if($COMMENTS['STATUS'] != "not_approved"){ echo "Currently ".$COMMENTS['STATUS']; }else{ echo "&nbsp;"; }
				echo "   </div>\n";
				
				
				
				echo "   <div class=\"post_name blue\">Product: ".$COMMENTS['PROD_NAME']."\n";
				echo "   </div>\n";
				...
				echo "   <p>".htmlspecialchars(nl2br(stripslashes(html_entity_decode($COMMENTS['COMMENT_TITLE'], ENT_QUOTES)))).": ".htmlspecialchars(nl2br(stripslashes(html_entity_decode($COMMENTS['COMMENT'], ENT_QUOTES))));
				
				echo "   <span class=\"post_name blue\"><br/><br/>Posted by: ".htmlspecialchars(stripslashes(html_entity_decode($COMMENTS['NAME'], ENT_QUOTES)))."\n";
				echo "   </span>\n";
	...
	if($at_least_one == 0){
		echo "<h1 style=\"margin: 5px;\"><b>There are no new cart comments.</b></h1>\n";
	}
...
?>
      ...
      <table id="tab2-content" border="0" cellspacing="0" cellpadding="0" class="feature_sub tab_content" style="display: none;">
         <tr>
            <td style="padding-left: 15px;width:810px;">

               <form name="comment_settings_form" id="comment_settings_form" method="post" action="cart_comments.php">
               ...
               <div id="form_body_container" style="text-align: left;background-color: transparent;margin: 10;padding: 0px;width: ;border-style: none;border-width: 0;border-color: 000;">
                  ...
                  <h1 style="margin-top: 10px; margin-bottom: 5px;">Cart Comment Settings</h1>
                  <p class="instructions" style="color: #595959; margin-bottom: 15px;">These settings define different parts of the cart comment system.</p>
                  ...
                  <div class="field-container">
                     <p class="myform-field_title-left"><label for="is_enabled"> Allow users to add product comments?</label></p>
                     <p class="myform-input_container"><input type="checkbox" name="is_enabled" id="is_enabled" style="margin-right: 10px;" <? echo $is_enabled; ?> /></p>
                     <p class="instructions" style="color: #595959; padding-top: 3px;">If this is not checked, users will not be able to submit comments about cart products.</p>
                  ...
                  <div class="field-container">
                     ...
                     <p class="myform-input_container"><input type="checkbox" name="require_approval" id="require_approval" style="margin-right: 10px;" <? echo $require_approval; ?> /></p>
                  ...
                  <p class="myform-input_container"><input type="text" name="email_to" id="email_to" style="width: 220px; margin-right: 5px;" value="<? echo $admin_email; ?>"/><br/>Who should get notified when a new comment is posted?</p>
                  <div class="ie_cleardiv"></div>
                  </div>
...
$instructions = lang("Manage all comments posted about your shopping cart's products.");

# Build into standard module template
$module = new smt_module($module_html);
$module->meta_title = "Cart Comments";
$module->add_breadcrumb_link("Shopping Cart Menu", "program/modules/mods_full/shopping_cart.php");
$module->add_breadcrumb_link("Cart Comments", "program/modules/mods_full/shopping_cart/cart_comments.php");
$module->icon_img = "skins/".$_SESSION['skin']."/icons/full_size/shopping-icon-med.png";
$module->heading_text = "Cart Comments";
